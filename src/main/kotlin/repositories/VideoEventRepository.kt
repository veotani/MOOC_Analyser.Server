...
class VideoEventRepository
{
    ...
    fun getVideoEventFileNames(): MutableList<String>
    {
        val res = mutableListOf<String>()
        val getVideoEventFileNamesQueue = "SELECT log_file_name FROM video_events GROUP BY log_file_name"
        val statement = this.connection!!.createStatement()
        statement.executeQuery(getVideoEventFileNamesQueue)
        val resultSet = statement.resultSet
        while (resultSet.next())
        {
    ...
    fun getLogFiles(): MutableList<String>
    {
        var result = mutableListOf<String>()
        val getLogFileNamesQuery = "SELECT log_file_name FROM video_events GROUP BY log_file_name"
        val statement = this.connection!!.createStatement()
        statement.executeQuery(getLogFileNamesQuery)
        val resultSet = statement.resultSet
        while (resultSet.next())
        {
            val fileName = resultSet.getString(1)
    ...
    fun getVideoIdForFile(fileName: String): List<String>
    {
        val result = mutableListOf<String>()
        val getVideoIdsByFile = "SELECT video_id FROM video_events WHERE log_file_name = \"$fileName\" " +
                "GROUP BY video_id"
        val statement = this.connection!!.createStatement()
        statement.executeQuery(getVideoIdsByFile)
        val resultSet = statement.resultSet
        while (resultSet.next())
        {
            val fileName = resultSet.getString(1)
    ...
    fun getUserNamesForFile(fileName: String): List<String>
    {
        val result = mutableListOf<String>()
        val getUserNamesForFile = "SELECT user_name FROM video_events WHERE log_file_name = \"$fileName\" " +
                "GROUP BY user_name"
        val statement = this.connection!!.createStatement()
        statement.executeQuery(getUserNamesForFile)
        val resultSet = statement.resultSet
        while (resultSet.next())
        {
            val fileName = resultSet.getString(1)
    ...
    fun countUsersInLogFile(fileName: String, userName: String): Int
    {
        val countUsersInLogs = "SELECT COUNT(user_name) FROM video_events WHERE user_name = \"$userName\"  AND " +
                "log_file_name = \"$fileName\""
        val statement = this.connection!!.createStatement()
        statement.executeQuery(countUsersInLogs)
        val resultSet = statement.resultSet
        while (resultSet.next())
            return resultSet.getString(1).toInt()
        return 0
    }
}
